...
DoStartZ80:
		btst    #6,(v_megadrive).w ; is system a 50hz device?
                ...
                cmpi.b  #5,(f_palmusic).w
                bne.s   @end
                move.b  #0,(f_palmusic).w
                bra.w   UpdateMusic
@end:
                addq.b  #1,(f_palmusic).w
                rts
...
PlaySegaSound:
		stopZ80
                lea	SegaPCM(pc),a2			; Load the SEGA PCM sample into a2. It's important that we use a2 since a0 and a1 are going to be used up ahead when reading the joypad ports
...
PlayPCM_Loop:
		...
		move.w	#2,d0				; Write the pitch ($14 in this case) to d0
		dbf	d0,*				; Decrement d0; jump to itself if not 0. (for pitch control, avoids playing the sample too fast)
		subq.w	#1,d3				; Subtract 1 from the PCM sample size
