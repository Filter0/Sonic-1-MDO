...
loc_E0:
	...
	endc
		...
		endc
Checksum:	dc.w 0 ; useless as checksum isn't nessesary anymore
		dc.b "J               " ; I/O support
...
VBla_Exit:
		addq.w	#1,(v_vbla_count).w
		movem.l	(sp)+,d0-a6
		rte
; ===========================================================================
VBla_Index:	dc.w VBla_00-VBla_Index, VBla_02-VBla_Index
		dc.w VBla_04-VBla_Index, sub_106E-VBla_Index
...
VBla_00:
	...
	@islevel:
		tst.b	(f_water).w ; does zone have water ?
...
; ---------------------------------------------------------------------------
; Subroutine to	load kosinski compressed level tiles
; ---------------------------------------------------------------------------
LoadZoneTiles:
		moveq	#0,d0			; Clear d0
		...
		move.w	a1,d3			; Move a word of a1 to d3, note that a1 doesn't exactly contain the address of v_256x256/StartOfRAM anymore, after KosPlusDec, a1 now contains v_256x256/StartOfRAM + the size of the file decompressed to it, d3 now contains the length of the file that was decompressed
...
; this is ported over from sonic 2 to work for sonic 1, saves a bit of ram but costs a flag

; loc_40804:
Build_HUD:
        tst.w	(v_rings).w
	beq.s	@1	; blink ring count if it's 0
	moveq	#0,d1
	btst	#3,(v_framebyte).w
	bne.s	@2	; only blink on certain frames
...
@2
	...
	lea	Map_HUD(pc),a1
	movea.w	#-$7936,a3	; set art tile and flags
	add.w	d1,d1
	adda.w	(a1,d1.w),a1
	moveq   #0,d1
        move.b	(a1)+,d1
...
; ---------------------------------------------------------------------------
Blk16_GHZ:	incbin	"map16\GHZ.kosp"
		even
Nem_Title:	incbin	"artnem\8x8 - Title.nem"	; GHZ primary patterns
		even
Nem_GHZ_2nd:	incbin	"artkos\8x8 - GHZ.kosp"	; GHZ secondary patterns
		even
Blk256_GHZ:	incbin	"map256\GHZ.kosp"
		even
Blk16_LZ:	incbin	"map16\LZ.kosp"
		even
Nem_LZ:		incbin	"artkos\8x8 - LZ.kosp"	; LZ primary patterns
		even
Blk256_LZ:	incbin	"map256\LZ.kosp"
		even
Blk16_MZ:	incbin	"map16\MZ.kosp"
		even
Nem_MZ:		incbin	"artkos\8x8 - MZ.kosp"	; MZ primary patterns
		even
Blk256_MZ:	incbin	"map256\MZ.kosp"
		even
Blk16_SLZ:	incbin	"map16\SLZ.kosp"
		even
Nem_SLZ:	incbin	"artkos\8x8 - SLZ.kosp"	; SLZ primary patterns
		even
Blk256_SLZ:	incbin	"map256\SLZ.kosp"
		even
Blk16_SYZ:	incbin	"map16\SYZ.kosp"
		even
Nem_SYZ:	incbin	"artkos\8x8 - SYZ.kosp"	; SYZ primary patterns
		even
Blk256_SYZ:	incbin	"map256\SYZ.kosp"
		even
Blk16_SBZ:	incbin	"map16\SBZ.kosp"
		even
Nem_SBZ:	incbin	"artkos\8x8 - SBZ.kosp"	; SBZ primary patterns
		even
Blk256_SBZ:	incbin	"map256\SBZ.kosp"
		even
; ---------------------------------------------------------------------------
...
Nem_TryAgain:	incbin	"artnem\Ending - Try Again.nem"
		even
Kos_EndFlowers:	incbin	"artkos\Flowers at Ending.kosp" ; ending sequence animated flowers
		even
